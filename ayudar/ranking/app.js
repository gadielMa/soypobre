(() => {
  ...
  async function load() {
    updateLabel();
    try {
      const { ranking = [], recent = [] } = await invoke();
      document.getElementById('rankingStatus').textContent = ranking.length ? 'Ranking actualizado con donaciones registradas.' : 'Todavía no hay donaciones registradas para este territorio.';
      ...
      activity.hidden = !recent.length;
      activityList.innerHTML = recent.map((entry) => `<article><span class="news-avatar green-avatar">${initials(entry.donor_name)}</span><p><strong>${escape(entry.donor_name)}</strong> registró una donación de <b>${pesos(entry.amount)}</b> para ${escape(entry.recipient_name)}.</p><small>${new Intl.RelativeTimeFormat('es', { numeric: 'auto' }).format(Math.round((new Date(entry.created_at) - Date.now()) / 60000), 'minute')}</small></article>`).join('');
    } catch (error) { document.getElementById('rankingStatus').textContent = error.message; }
  }
